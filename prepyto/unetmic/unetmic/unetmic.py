...
def agumentaion_predict(image, unet3d):
    results=[]
    for i in range(4):
        temp_predict= np.rot90(np.rot90(image, 1, (1,2))  ,i, (0, 1))
        results.append(run_segmentation(temp_predict,unet3d))
        results[-1]=np.rot90(np.rot90(results[-1], -1*i, (0,1))  ,-1, (1, 2))

        temp_predict = np.rot90(np.rot90(image, 2, (0, 2)) ,i, (1, 2))
        results.append(run_segmentation(temp_predict,unet3d))
        results[-1]=np.rot90(np.rot90(results[-1], -1 * i, (1, 2)), -2, (0, 2))
...
def run_segmentation(image, unet3d):
    # region of . Middle image region kept after segmentation
    ...
    for i in tqdm(range(0, image.shape[0], roi)):
        ...
        for j in range(0, image.shape[1], roi):
            ...
            for k in range(0, image.shape[2], roi):
                # print(k)
                numit += 1
                # temp = image_pd[i:i + wsize, j:j + wsize, k:k + wsize][np.newaxis, :, :, :, np.newaxis]
                # test = agumentaion_predict(temp,unet3d)
