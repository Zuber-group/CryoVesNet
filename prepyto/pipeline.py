...
class Pipeline():
    ...
    def label_vesicles_simply(self, input_array_name='deep_mask', threshold_coef=1, within_segmentation_region=True,
        ...
        deep_labels = skimage.morphology.label(deep_labels)
        self.deep_labels = deep_labels.astype(np.uint16)
        # if within_segmentation_region==True:
        #     self.set_array('cytomask')
        #     self.deep_labels = prepyto.sround_remover(self.deep_labels, self.cytomask)
    ...
    def repair_spheres(self, memkill = True):
        ...
        # deep_labels = mrcfile.open(self.deep_labels_path).data
        # sphere_labels = mrcfile.open(self.sphere_labels_path)
        # sphere_labels = (sphere_labels.data >= 1)
        # sphere_labels = skimage.morphology.label(sphere_labels)  # connectivity one is better

        deep_labels = self.deep_labels.copy()
        sphere_labels = self.sphere_labels.copy()
        # sphere_labels = (sphere_labels >=1)
        # sphere_labels = skimage.morphology.label(sphere_labels)
        print(self.min_vol)
        temp_best_corrected_labels = prepyto.sround_remover(sphere_labels, self.cytomask,self.min_vol)
        print('why')

        # temp_best_corrected_labels = (temp_best_corrected_labels >= 1)
        # temp_best_corrected_labels = skimage.morphology.label(temp_best_corrected_labels)
        # temp_best_corrected_labels = prepyto.oneToOneCorrection(deep_labels, sphere_labels)
        # temp_best_corrected_labels = prepyto.oneToOneCorrection(deep_labels, temp_best_corrected_labels)

        # temp_best_corrected_labels = skimage.morphology.label(temp_best_corrected_labels)
        self.convex_labels = temp_best_corrected_labels

        # self.sphere_labels = prepyto.pacman_killer(temp_best_corrected_labels)
        # self.label_convexer(temp_best_corrected_labels)
    ...
    def evaluation(self, reference_path=None, prediction_path=None):
        ...
        # corrected_labels = corrected_labels * self.cytomask
        # corrected_labels_mask = mrc_cleaner.interactive_cleaner(self.image,corrected_labels)
        corrected_labels_mask = corrected_labels >= 1
        # print(corrected_labels_mask)
    ...
    def clean_evaluation(self, reference_path=None, prediction_path=None):
        self.set_array("cytomask")
        # if input_array_name == 'last_output_array_name':
        #     input_array_name = self.last_output_array_name
        # self.set_array(input_array_name)
        ...
        mask = mask >= 10
        corrected_labels = prediction.data
        corrected_labels = skimage.transform.resize(corrected_labels, output_shape=np.shape(self.image),
                                 preserve_range=True).astype(np.float32)

        corrected_labels = corrected_labels * self.cytomask
        corrected_labels_mask = mrc_cleaner.interactive_cleaner(self.image,corrected_labels)
        corrected_labels_mask = (corrected_labels_mask > 0)
        # print(corrected_labels_mask)
        evaluator = evaluation_class.ConfusionMatrix(corrected_labels_mask, mask)
        # print(evaluator)
    ...
    def automatic_evaluation(self, reference_path=None, prediction_path=None):
        self.set_array("cytomask")
        self.set_array("deep_mask")
        self.set_array("sphere_labels")
        ...
        evaluator1 = evaluation_class.ConfusionMatrix(self.deep_mask, mask)
        evaluator2 = evaluation_class.ConfusionMatrix(self.sphere_labels>=1, mask)
        evaluator3 = evaluation_class.ConfusionMatrix(self.convex_labels>=1, mask)
        # print(evaluator)
