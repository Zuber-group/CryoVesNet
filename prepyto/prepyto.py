...
def save_label_to_tiff(labels, path_to_file, folder_to_save, suffix):
    skimage.io.imsave(
        os.path.normpath(folder_to_save) + '/' + os.path.splitext(os.path.split(path_to_file)[1])[0] + suffix + '.tiff',
        labels, plugin='tifffile')
    return os.path.normpath(folder_to_save) + '/' + os.path.splitext(os.path.split(path_to_file)[1])[
        0] + suffix + '.tiff'
...
def find_threshold_per_vesicle_intensity(int_image, image_label, mask_image, dilation, convex,
                                         minimum_volume_of_vesicle):
...
def objectwise_evalution(reff, prediction, delta_size=1,proportion=0.0):
    ...
    for i in range(0, len(reff_regions['label'])):
        ...
        reff_center = np.array([reff_regions['centroid-0'][i],reff_regions['centroid-1'][i],reff_regions['centroid-2'][i]])
        ...
        if len(unique)>0:
            if counts[-1] >= proportion*np.count_nonzero(sub_old_label_mask):
                ...
                p = int(round((predict_regions['bbox-3'][index_related_label] - predict_regions['bbox-0'][index_related_label]) ))
                q = int(round((predict_regions['bbox-4'][index_related_label] - predict_regions['bbox-1'][index_related_label]) ))
                r = int(round((predict_regions['bbox-5'][index_related_label] - predict_regions['bbox-2'][index_related_label]) ))
    
                predicted_diameter = np.max([p, q, r])
                predicted_center =  np.array([predict_regions['centroid-0'][index_related_label],predict_regions['centroid-1'][index_related_label],predict_regions['centroid-2'][index_related_label]])
                a=min(reff_diameter,predicted_diameter)
                b=max(reff_diameter,predicted_diameter)
                c= 1-a/b
                # all += [[reff_regions['label'][i], related_label ,reff_diameter, predicted_diameter, c ,math.sqrt(sum((reff_center - predicted_center) ** 2))]]
    ...
    # important =[np.mean(tab, axis=0).tolist(),np.std(tab, axis=0).tolist(),ttest_1samp(tab[:,0],0),ttest_1samp(tab[:,1],0),ttest_1samp(tab[:,2],0),ttest_ind(tab[:,0],tab[:,1]),ttest_ind(tab[:,0],tab[:,2]),ttest_ind(tab[:,1],tab[:,2])]
...
def oneToOneCorrection(old_label, new_label, delta_size=3):
    ...
    # vesicles= [] 
    vesicles = [0 for t in range(len(vesicle_regions['label']))]
    print(len(vesicle_regions['label']))
    for i in range(0, len(vesicle_regions['label'])):
        sub_old_label = old_label[
                        vesicle_regions['bbox-0'][i] - delta_size: vesicle_regions['bbox-3'][i] + delta_size + 1,
        ...
        if any(np.isnan([a, b, c])):
            ...
            vesicles[i] = sub_new_label[a, b, c]
    ...
    for j in intersected[1:]:
        ...
        print(places)
        places = places[0]
        jj = np.where(myvesicle_regions['label'] == j)[0][0]
        print(jj)
        sub_temp_label = new_label[
                         myvesicle_regions['bbox-0'][jj] - delta_size: myvesicle_regions['bbox-3'][jj] + delta_size + 1,
                         myvesicle_regions['bbox-1'][jj] - delta_size: myvesicle_regions['bbox-4'][jj] + delta_size + 1,
                         myvesicle_regions['bbox-2'][jj] - delta_size: myvesicle_regions['bbox-5'][jj] + delta_size + 1]
        sub_temp_label = sub_temp_label == j
        pxj, pyj, pzj = np.where(sub_temp_label)
        corrected_labels[pxj + myvesicle_regions['bbox-0'][jj] - delta_size,
                         pyj + myvesicle_regions['bbox-1'][jj] - delta_size,
                         pzj + myvesicle_regions['bbox-2'][jj] - delta_size] = 0
        for m in places:
            print(j, m)
            sub_orig_label = old_label[
                             vesicle_regions['bbox-0'][m] - delta_size: vesicle_regions['bbox-3'][m] + delta_size + 1,
            ...
            corrected_labels[px + vesicle_regions['bbox-0'][m] - delta_size,
                             ...
                             pz + vesicle_regions['bbox-2'][m] - delta_size] = m + 1000

    # best_corrected_labels = (corrected_labels >= 1) | (old_label >= 1)
    best_corrected_labels = (corrected_labels >= 1)
    # best_corrected_labels = corrected_labels
...
def get_sphere_dataframe(image, image_label, margin=5):
    ...
    for i in tqdm(range(len(vesicle_regions)), desc="fitting sphere to vesicles"):
        ...
        density, keep_label, new_centroid, new_radius, thickness = get_sphere_parameters(image, label, margin, radius,
                                                                                         rounded_centroid)
        # print(label , radius , new_radius)
        # print((sum(np.array(new_centroid-rounded_centroid)**2)))
        if (sum(np.array(new_centroid-rounded_centroid)**2)>100) or new_radius>=1.5*radius:
            print(label, sum(np.array(new_centroid - rounded_centroid) ** 2))
            ...
            densities.append([])
            radii.append(radius)
            centers.append(rounded_centroid)
...
def get_sphere_parameters(image, label, margin, radius, rounded_centroid):
    image_box = extract_box_of_radius(image, rounded_centroid, radius + margin)
    need_reextract = True
    max_shift = 0.5 * np.linalg.norm(image_box.shape)
    total_shift = np.zeros(3)
    i = 0
    while need_reextract and (i < 20):
...
def expand_small_labels(deep_mask, labels, initial_threshold, min_vol):
    ...
    step = 0.025
    for threshold in tqdm(np.arange((initial_threshold - step), 0.8, -step),
                          desc="Expanding labels until none is too small"):
...
def gipfeli_solver(deep_mask, deep_labels,ves_table, threshold ,delta_size = 1):
    pass
...
def get_optimal_sphere_radius_from_radial_profile(radial_profile):
    ...
    optimal_radius = i_membrane_center + ndimage.gaussian_filter1d(derivative2[i_membrane_center:i_membrane_outer_halo],
                                                                   1).argmin()
